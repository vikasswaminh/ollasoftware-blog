...
The Astro-powered blog for **OllaSoftware**, published at **https://blogs.ollasoftware.com**.
...
- Push to `main` → GitHub Actions on an NH **self-hosted runner** (`nh-marketing`, LXC 505) →
  `npm run build` → `wrangler pages deploy` to the Cloudflare Pages project **`ollasoftware-blog`** →
  live at `blogs.ollasoftware.com`.
